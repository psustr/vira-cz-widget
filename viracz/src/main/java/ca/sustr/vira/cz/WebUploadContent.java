...
public class WebUploadContent extends AsyncTask<Void, Void, Void>
{
	...
	public WebUploadContent(Context context, WidgetService widgetService, int viewID)
	{
		m_context		= context;
	...
	protected Void doInBackground(Void... params)
	{
		...
		try
		{
			...
			if (m_viewID == R.id.quoteOfTheDay)
			{
				...
				outFileName = m_context.getResources().getString(R.string.quoteFileName);
			}
			else
			{
				string = m_widgetService.getTopic();
				outFileName = m_context.getResources().getString(R.string.topicFileName);
			}
