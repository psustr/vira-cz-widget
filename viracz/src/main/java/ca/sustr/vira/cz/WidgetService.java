...
public class WidgetService extends JobIntentService
{
	...
	private	ComponentName				m_me;
	private	AppWidgetManager			m_mgr;
	...
	@Override
	// Called when the service is first created, including during a phone reboot
	public void onCreate()
	{
        // This line is MANDATORY! Removal will cause a null pointer exception!
        super.onCreate();

		// Initialize context variables
		m_context 	= this;
		m_me 		= new ComponentName(this, Widget.class);
		m_mgr 		= AppWidgetManager.getInstance(m_context);
		m_prefs		= PreferenceManager.getDefaultSharedPreferences(m_context);
		m_alert		= false;

		// On Android 2.2 the AsyncTask class needs to be instantiated in the
        // main thread. This code is not necessary in Android 4.2. Weird!
    ...
    {
        if (intent.getAction().equals(this.getString(R.string.ONWIDGET_AUTOUPDATE_ACTION)))
        {
            // TODO: Debug logs are compiled in but stripped at runtime
            Log.d("onHandleIntent", "Automatic update");

            // A null action is passed whenever the service is called with a
            // new intent which does not forward an action, e.g. during
            // onUpdate() in Widget.java
        ...
        if (intent.getAction().equals(this.getString(R.string.ONBACKPRESSED_PREFS_ACTION)))
        {
            // Pressed Back in Preferences, update the widget
            updateWidget();
            return;
        }

        if (intent.getAction().equals(this.getString(R.string.ONWIDGET_MANUALUPDATE_ACTION)))
        {
            // TODO: Debug logs are compiled in but stripped at runtime
            Log.d("onHandleIntent", "Manual update");
	...
	public void onReceiveDownload(int resourceID, String result)
	{
        switch (resourceID)
		{
	...
	{
		RemoteViews updateViews = new RemoteViews(m_context.getPackageName(), R.layout.widget);

		// 1. SETUP INTENTS
		// =====================================================================
        // Clicking on Quote displays a dialog
        Intent a1 = new Intent(this, DialogActivity.class);        
		a1.setAction(m_context.getString(R.string.ONCLICK_QUOTE_ACTION));
		a1.putExtra(getString(R.string.BUNDLE_KEY), getChapter());

		// FLAG_CANCEL_CURRENT is necessary in order for the extras to be sent
		PendingIntent pi1 = PendingIntent.getActivity(m_context, 0, a1,
        ...
        Intent a2 = new Intent(this, DialogActivity.class);        
		a2.putExtra(getString(R.string.BUNDLE_KEY), getTopic());
		a2.setAction(m_context.getString(R.string.ONCLICK_TOPIC_ACTION));

		// FLAG_CANCEL_CURRENT is necessary in order for the extras to be sent
		PendingIntent pi2 = PendingIntent.getActivity(m_context, 0, a2,
                PendingIntent.FLAG_CANCEL_CURRENT | PendingIntent.FLAG_IMMUTABLE);
		...
		// =====================================================================
		// Set font size
		String string = m_prefs.getString("widget_font_size", null);
		if (string != null)
		{
		...
		// Set font colour
		Integer value = m_prefs.getInt("widget_font_colour", Color.LTGRAY);
		if (value != null)
		{
		...
		// PERFORM THE UPDATE
		// =====================================================================
    	m_mgr.updateAppWidget(m_me, updateViews);
	}

	private void downloadQuote()
	{
		WebDownloadContent httpDownload = new WebDownloadContent(this, R.id.quoteOfTheDay);
		httpDownload.execute(m_context.getString(R.string.quoteUrl) +
                             m_context.getString(R.string.quoteFileName));
	}

	private void downloadQuoteDetail()
	{
		String  book 	= "";
		String	bookRef = "";
		String  chapter = "";
		...
		Element 	el 	= doc.getElementById(m_context.getString(R.string.quoteRefElemId));
		...
		else
		{
			Toast.makeText(getApplicationContext(),
					getString(R.string.error_quote_title) + el.text().trim() +
					getString(R.string.error_quote_syntax),
		...
		{
			Toast.makeText(getApplicationContext(),
					getString(R.string.error_book_title) + book +
					getString(R.string.error_book_unknown),
					Toast.LENGTH_LONG).show();
			return;
		}

		// Construct the URL and start the download
		url = m_context.getString(R.string.quoteDetailUrl) + bookRef + "/" + chapter;
		WebDownloadContent httpDownload = new WebDownloadContent(this, R.id.dialogTextView);
		httpDownload.execute(url);
	}	

	private void downloadTopic()
	{
		WebDownloadContent httpDownload = new WebDownloadContent(this, R.id.topicOfTheWeek);
		httpDownload.execute(m_context.getString(R.string.topicUrl) +
                             m_context.getString(R.string.topicFileName));
	}

	private void uploadQuote()
	{
		WebUploadContent ftpUpload = new WebUploadContent(this, R.id.quoteOfTheDay);
		ftpUpload.execute();
	}

	private void uploadTopic()
	{
		WebUploadContent ftpUpload = new WebUploadContent(this, R.id.topicOfTheWeek);
		ftpUpload.execute();
	}
	
	// Sets "Quote of the Day"
	private void parseQuote(String htmlString)
	{
		try
        {
            // Ignore empty data
            ...
            Element el1  = doc.getElementById(m_context.getString(R.string.quoteTextElemId));
            Element el2  = doc.getElementById(m_context.getString(R.string.quoteRefElemId));
            ...
            if ((el1 != null) && (el2 != null))
            {
                ...
                if (m_alert)
                {
                    ...
                    Toast.makeText(m_context, m_context.getString(R.string.quote_updated), Toast.LENGTH_SHORT).show();
                }
            }
            else
            {
        ...
        {
            // Alert the user but keep the alert flag set (will be unset later)
            Toast.makeText(m_context, m_context.getString(R.string.quote_empty), Toast.LENGTH_SHORT).show();
        }
	}

	// Sets biblical chapter detail
	private void parseChapter(String htmlString)
	{
		// Ignore empty data
		if ((htmlString == "") || (htmlString == null))
		{
		...
		// book selector)
		doc.select(getString(R.string.quoteDetailJunk1)).remove();
		doc.select(getString(R.string.quoteDetailJunk2)).remove();

        // Ignore empty and identical data
        Element 	el 	= doc.getElementById(m_context.getString(R.string.quoteDetailElemId));
	...
	private void parseTopic(String htmlString)
	{
		try
        {
            // Ignore empty data
            ...
            Document doc = Jsoup.parse(htmlString);
            Element el1  = doc.getElementById(m_context.getString(R.string.topicTextElemId));
            Element el2  = doc.getElementById(m_context.getString(R.string.topicDateElemId));
            ...
            if ((el1 != null) && (el2 != null))
            {
                ...
                if (m_alert)
                {
                    ...
                    Toast.makeText(m_context, m_context.getString(R.string.topic_updated), Toast.LENGTH_SHORT).show();
                    m_alert = false;
                }
            }
            else
		...
		catch (Exception e)
        {
            ...
            Toast.makeText(m_context, m_context.getString(R.string.topic_empty), Toast.LENGTH_SHORT).show();
            m_alert = false;
        }
	}
	...
	{
		m_quoteShort = m_prefs.getString("m_quoteShort", m_quoteShort);
		m_topicShort = m_prefs.getString("m_topicShort", m_topicShort);
		m_quote = m_prefs.getString("m_quote", m_quote);
		m_chapter = m_prefs.getString("m_chapter", m_chapter);
		m_topic = m_prefs.getString("m_topic", m_topic);
	}
